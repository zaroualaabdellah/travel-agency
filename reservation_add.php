...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
  ...
  <form method="post" action="">
    <div class="mb-3">
      ...
      <input type="date" id="date_reservation" name="date_reservation" required class="form-control" />
    </div>
    ...
    <div class="form-check mb-3">
      <input type="checkbox" id="chambre_supplementaire" name="chambre_supplementaire" class="form-check-input" />
    ...
    <div class="mb-3">
      ...
      <select name="id_client" id="id_client" class="form-select" required>
        ...
        <?php while ($client = $clientsRes->fetch_assoc()): ?>
          <option value="<?= $client['id_client'] ?>"><?= htmlspecialchars($client['nom'] . ' ' . $client['prenom']) ?></option>
        <?php endwhile; ?>
      </select>
    </div>
